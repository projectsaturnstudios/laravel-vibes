...
class AuthenticateAgentsUser
{
    ...
    public function handle(Request $request, Closure $next) : mixed
    {
        ...
        {
            if($session_id = request()->get('session_id') ?? null)
            {
                if(is_null($sesh = VibeSesh::load($session_id))) abort(402);
                if($user = $sesh->getUser())
                {
                    auth($sesh->morph)->login($user);
                }
                else
                {
                    abort(401);
